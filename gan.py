...
class ConvRNN2D(RNN):
    ...
    def get_initial_state(self, inputs):
        ...
        int_shape = initial_state.shape
        out_shape = [int_shape[0], int_shape[1]*2, int_shape[2]*2, int_shape[3]]
        initial_state = self.cell.input_conv(initial_state, array_ops.zeros(tuple(shape), initial_state.dtype),
...
class ConvTransposeLSTM2DCell(DropoutRNNCellMixin, Layer):
    """
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(ConvTransposeLSTM2DCell, self).__init__(**kwargs)
    ...
    def call(self, inputs, states, training=None):
        ...
        x_i = self.input_conv(inputs_i, kernel_i, out_shape, bias_i, padding=self.padding)
        x_f = self.input_conv(inputs_f, kernel_f, out_shape, bias_f, padding=self.padding)
        x_c = self.input_conv(inputs_c, kernel_c, out_shape, bias_c, padding=self.padding)
        x_o = self.input_conv(inputs_o, kernel_o, out_shape, bias_o, padding=self.padding)
        h_i = self.recurrent_conv(h_tm1_i, recurrent_kernel_i, out_shape)
        h_f = self.recurrent_conv(h_tm1_f, recurrent_kernel_f, out_shape)
        h_c = self.recurrent_conv(h_tm1_c, recurrent_kernel_c, out_shape)
        h_o = self.recurrent_conv(h_tm1_o, recurrent_kernel_o, out_shape)
    ...
    def input_conv(self, x, w, o_shape, b=None, padding='valid'):
        conv_out = K.conv2d_transpose(x, w, strides=self.strides, padding=padding, output_shape=o_shape,
    ...
    def recurrent_conv(self, x, w, o_shape):
        conv_out = K.conv2d_transpose(x, w, strides=(1, 1), padding='same', output_shape=o_shape,
                                      data_format=self.data_format)
        return conv_out
...
class ConvTransposeLSTM2D(ConvRNN2D):
    """Convolutional LSTM.
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        cell = ConvTransposeLSTM2DCell(filters=filters,
        ...
        super(ConvTransposeLSTM2D, self).__init__(cell,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        return super(ConvTransposeLSTM2D, self).call(inputs,
                                            mask=mask,
                                            training=training,
...
class ToyGenerator(tf.keras.Model):
    ...
    def __init__(self, batch_size, time_steps, Dz, Dx, state_size, filter_size, output_activation='sigmoid', bn=False,
        ...
        k_init = None
        self.encoder.add(layers.LSTM(self.state_size, return_sequences=False, recurrent_initializer=k_init,
        ...
        self.decoder.add(layers.LSTM(self.state_size, return_sequences=False, recurrent_initializer=k_init,
                                     kernel_initializer=k_init))
    ...
    def call(self, inputs, y=None, training=True, mask=None):
        z = tf.reshape(tensor=inputs, shape=[self.batch_size, self.time_steps, self.Dx])
        if y is not None:
            y = tf.broadcast_to(y[:, None, :], [self.batch_size, self.time_steps, self.Dy])
            z = tf.concat([z, y], -1)

        # LSTM encoder
        last_h = self.encoder(z, training=training)
        last_h = tf.expand_dims(last_h, axis=1)

        # LSTM + FC decoder
        y = tf.zeros([self.batch_size, 1, self.Dx])
        ht = tf.zeros([self.batch_size, 1, self.state_size])
        for n in range(1, self.time_steps+1):
            inp = tf.concat([tf.reshape(y[:, n-1, :], [self.batch_size, 1, self.Dx]), last_h, ht], axis=-1)
            ht = self.decoder(inp, training=training)
            ht = tf.expand_dims(ht, axis=1)
            inp_y = tf.concat([tf.reshape(y[:, n-1, :], [self.batch_size, 1, self.Dx]), last_h, ht], axis=-1)
            yt = self.fc(inp_y, training=training)
            y = tf.concat([y, yt], axis=1)

        x = tf.reshape(tensor=y[:, 1:, :], shape=[self.batch_size, self.time_steps, self.Dx])
        return x
...
class VideoDCG(tf.keras.Model):
    ...
    def call_all(self, inputs_z, inputs_y, inputs_real, training=True, mask=None):
...
class VideoGenerator(tf.keras.Model):
    ...
    def __init__(self, batch_size, int_time_steps, pred_time_steps, state_size, x_width, x_height, z_width=5, z_height=5,
                 y_width=5, y_height=5, filter_size=64, bn=False, output_activation="sigmoid", nlstm=1,
                 cat=False, nchannel=3):
        ...
        # ConvLSTM encoder
        #
        self.encoder1 = tf.keras.layers.ConvLSTM2D(self.filter_size, kernel_size=(5, 5), strides=(2, 2), padding='same',
                                          data_format='channels_last', return_state=True, return_sequences=True)
        self.bn1 = tf.keras.layers.BatchNormalization()

        self.encoder2 = tf.keras.layers.ConvLSTM2D(self.filter_size*2, kernel_size=(5, 5), strides=(2, 2), padding='same',
                                          data_format='channels_last', return_state=True, return_sequences=True)
        self.bn2 = tf.keras.layers.BatchNormalization()

        self.encoder3 = tf.keras.layers.ConvLSTM2D(self.filter_size*4, kernel_size=(5, 5), strides=(2, 2), padding='same',
                                          data_format='channels_last', return_state=True, return_sequences=True)
        # self.bn3 = tf.keras.layers.BatchNormalization()
        #
        # decoder
        #
        self.decoder1 = tf.keras.layers.ConvLSTM2D(self.filter_size*4, kernel_size=(3, 3), strides=(1, 1),
                                                   padding='same', data_format='channels_last', return_state=True,
                                                   return_sequences=True)
        self.bn4 = tf.keras.layers.BatchNormalization()

        self.decoder2 = tf.keras.layers.ConvLSTM2D(self.filter_size*2, kernel_size=(5, 5), strides=(1, 1),
                                                   padding='same', data_format='channels_last',
                                                   return_state=True, return_sequences=True)
        self.bn5 = tf.keras.layers.BatchNormalization()

        # self.decoder3 = tf.keras.layers.ConvLSTM2D(self.filter_size, kernel_size=(6, 6), strides=(1, 1),
        #                                            padding='same', data_format='channels_last',
        #                                           return_state=True, return_sequences=True)

        self.decoder3 = tf.keras.layers.Conv2DTranspose(self.filter_size, (5, 5), strides=(2, 2), padding='same',
                                                        use_bias=False, activation=output_activation,
                                                        data_format='channels_last')
        self.bn6 = tf.keras.layers.BatchNormalization()
        self.decoder4 = tf.keras.layers.Conv2DTranspose(self.nchannel, (5, 5), strides=(2, 2), padding='same',
                                                        use_bias=False, activation=output_activation,
                                                        data_format='channels_last')

    def call_all(self, inputs_z, inputs_y, inputs_real, training=True):
        # x input sequence
        x = tf.reshape(tensor=inputs_real, shape=[self.batch_size, self.x_height, self.int_time_steps, self.x_width,
                                                  self.nchannel])
        x = tf.transpose(x, (0, 2, 1, 3, 4))
        # z has shape of [batch_size, time_step, sub_sequence_hidden_dims]
        z = tf.reshape(tensor=inputs_z, shape=[self.batch_size, self.pred_time_steps, self.z_height, self.z_width, 1])
        y = tf.broadcast_to(inputs_y[:, None, :], [self.batch_size, self.pred_time_steps, self.y_height, self.y_width])
        y = tf.reshape(tensor=y, shape=[self.batch_size, self.pred_time_steps, self.y_height, self.y_width, 1])
        zy = tf.concat([z, y], -1)

        # return the last state that contains all information about the input sequence
        output_seq1, input_h, input_c = self.encoder1(x)
        if self.rnn_bn:
            output_seq1 = self.bn1(output_seq1)
        output_seq2, input_h, input_c = self.encoder2(output_seq1)
        if self.rnn_bn:
            output_seq2 = self.bn2(output_seq2)
        output_seq3, input_h, input_c = self.encoder3(output_seq2)

        # se1, h, c = ConvTransposeLSTM2D(self.filter_size, kernel_size=(5, 5), strides=(2, 2), padding='same',
        #                                data_format='channels_last', return_state=True,
        #                                return_sequences=True)(output_seq3)

        padded_zy = tf.keras.layers.ZeroPadding3D(padding=(0, 1, 1))(zy)

        output_seq4, h, c = self.decoder1([padded_zy, input_h, input_c])
        ...
        output_seq5, h, c = self.decoder2(padded_seq4)

        if self.rnn_bn:
            output_seq5 = self.bn5(output_seq5)

        # padded_seq5 = tf.keras.layers.ZeroPadding3D(padding=(0, 8, 8))(output_seq5)
        # output_seq6, h, c = self.decoder3(padded_seq5)
        # padded_seq6 = tf.keras.layers.ZeroPadding3D(padding=(0, 16, 16))(output_seq6)

        conv_inputs = tf.reshape(output_seq5, [self.batch_size * self.pred_time_steps, 16, 16, self.filter_size*2])
        y = self.decoder3(conv_inputs)
        y = self.decoder4(y)

        y = tf.reshape(y, [self.batch_size, self.pred_time_steps, self.x_height, self.x_width, self.nchannel])
        y = tf.transpose(y, (0, 2, 1, 3, 4))
        y = tf.reshape(tensor=y, shape=[self.batch_size, self.x_height, self.x_width * self.pred_time_steps,
                                        self.nchannel])
        return zy, input_h, y
...
class VideoDiscriminator(tf.keras.Model):
    ...
    def __init__(self, batch_size, time_steps, state_size, x_width, x_height, z_width=5, z_height=5,
                 y_width=5, y_height=5, filter_size=64,
                 bn=False, output_activation="sigmoid", nlstm=1, cat=False, nchannel=3):
