...
parser.add_argument('-d', '--dname', type=str, default='animation', choices=['animation', 'human_action'])
...
parser.add_argument('-p', '--path', type=str, default='/home/tianlin_xu/dataset/animation/*_data.tfrecord')
...
def main():
    ...
    data_processor = data_utils.DataProcessor(filenames, time_steps, channels)
    batched_x = data_processor.provide_video_data(buffer, batch_size * 2, x_height, x_width)

    # adjust channel parameter as we want to drop the alpha channel for animated Sprites
    if dname == 'animation':
        channels = channels - 1
    ...
    lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=gen_lr, decay_steps=500,
    ...
    generator = gan.VideoDCG(batch_size, time_steps, x_width, x_height, z_width, z_height, g_state_size,
                             filter_size=g_filter_size, bn=bn, nlstm=nlstm, nchannel=channels)

    discriminator_h = gan.VideoDCD(batch_size, time_steps, x_width, x_height, z_width, z_height, d_state_size,
                                   filter_size=d_filter_size, bn=bn, nchannel=channels)
    discriminator_m = gan.VideoDCD(batch_size, time_steps, x_width, x_height, z_width, z_height, d_state_size,
                                   filter_size=d_filter_size, bn=bn, nchannel=channels)
    ...
    saved_file = "{}_{}{}-{}:{}:{}.{}".format(dataset, datetime.now().strftime("%h"), datetime.now().strftime("%d"),
    ...
    model_fn = "%s_Dz%d_Dy%d_bs%d_gss%d_gfs%d_dss%d_dfs%d_eps%d_l%d_p%d_lr%d_nl%d_s%02d" % \
               (dname, args.z_dims_t, args.y_dims, batch_size, g_state_size, g_filter_size, d_state_size, d_filter_size,
                np.round(np.log10(sinkhorn_eps)), sinkhorn_l, np.round(np.log10(reg_penalty)),
